...
from src.filters.vsnlms import VSNLMS
...
class AdaptiveFilter:
    """
    Production-oriented adaptive filtering orchestration layer.

    Responsibilities:

    - adaptive DSP training
    ...
    def apply_filter(
        ...
        try:
            ...
            logger.error(
                "Adaptive runtime filtering failed: %s",
                runtime_error,
            )
            raise
    ...
    def plot_performance(self) -> None:
        if not self.rmse_history:
            logger.warning("No convergence metrics available.")
            return
