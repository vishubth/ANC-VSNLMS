...
class VSNLMS:
    ...
    def update_weights(self, x, d):
        ...
        # This is calculated after updating weights, using the norm of the weights vector itself
        weight_norm = np.linalg.norm(self.weights)
        # print(f"Current weight norm: {weight_norm}")
...
def read_audio_file(filename):
    ...
    if len(data.shape) == 2:  # Check if the audio is stereo
        data = np.mean(data, axis=1)  # Convert stereo to mono by averaging both 
    # Normalize the input
